...
class Controller:
    def __init__(self, root, role):
        ...
        self.role = role  # Role: 'admin', 'employee', or 'user'
    ...
    def setup_event_handlers(self):
        # Bind treeview selections (view-only, allowed for all roles)
        ...
        # Product tab buttons
        product_frame = self.view.products_tab.children['!labelframe2'].children['!frame']
        if self.check_permission('add'):
            product_frame.children['!button'].configure(command=self.add_product)
        else:
            product_frame.children['!button'].configure(state='disabled')
        if self.check_permission('update'):
            product_frame.children['!button2'].configure(command=self.update_product)
        else:
            product_frame.children['!button2'].configure(state='disabled')
        if self.check_permission('delete'):
            product_frame.children['!button3'].configure(command=self.delete_product)
        else:
            product_frame.children['!button3'].configure(state='disabled')
        product_frame.children['!button4'].configure(command=self.view.clear_product_fields)  # Clear is allowed for all

        # Supplier tab buttons
        supplier_frame = self.view.suppliers_tab.children['!labelframe2'].children['!frame']
        if self.check_permission('add'):
            supplier_frame.children['!button'].configure(command=self.add_supplier)
        else:
            supplier_frame.children['!button'].configure(state='disabled')
        if self.check_permission('update'):
            supplier_frame.children['!button2'].configure(command=self.update_supplier)
        else:
            supplier_frame.children['!button2'].configure(state='disabled')
        if self.check_permission('delete'):
            supplier_frame.children['!button3'].configure(command=self.delete_supplier)
        else:
            supplier_frame.children['!button3'].configure(state='disabled')
        supplier_frame.children['!button4'].configure(command=self.view.clear_supplier_fields)

        # Warehouse tab buttons
        # Warehouse tab buttons
        warehouse_frame = self.view.warehouses_tab.children['!labelframe2'].children['!frame']
        if self.check_permission('add'):
            warehouse_frame.children['!button'].configure(command=self.add_warehouse)
        else:
            warehouse_frame.children['!button'].configure(state='disabled')
        if self.check_permission('update'):
            warehouse_frame.children['!button2'].configure(command=self.update_warehouse)
        else:
            warehouse_frame.children['!button2'].configure(state='disabled')
        if self.check_permission('delete'):
            warehouse_frame.children['!button3'].configure(command=self.delete_warehouse)
        else:
            warehouse_frame.children['!button3'].configure(state='disabled')
        warehouse_frame.children['!button4'].configure(command=self.view.clear_warehouse_fields)

        # Inventory tab buttons (view-only, allowed for all)
        inventory_frame = self.view.inventory_tab.children['!labelframe']
        inventory_frame.children['!button'].configure(command=self.search_inventory)
        inventory_frame.children['!button2'].configure(command=self.show_all_inventory)
        inventory_frame.children['!button3'].configure(command=self.show_low_stock)

        # Stock entry tab buttons
        stock_frame = self.view.stock_entry_tab.children['!labelframe'].children['!frame']
        if self.check_permission('add'):
            stock_frame.children['!button'].configure(command=self.add_stock_entry)
        else:
            stock_frame.children['!button'].configure(state='disabled')
        stock_frame.children['!button2'].configure(command=self.view.clear_stock_entry_fields)

        # Reports tab buttons (view-only, allowed for all)
        reports_frame = self.view.reports_tab.children['!labelframe']
        reports_frame.children['!button'].configure(command=self.show_low_stock_report)
        reports_frame.children['!button2'].configure(command=self.show_stock_value_report)
        reports_frame.children['!button3'].configure(command=self.show_stock_per_warehouse)
        reports_frame.children['!button4'].configure(command=self.show_supplier_delivery_history)
        reports_frame.children['!button5'].configure(command=self.show_transaction_history)
        reports_frame.children['!labelframe'].children['!button'].configure(command=self.apply_date_range)
    ...
    def add_product(self):
        if not self.check_permission('add'):
            messagebox.showerror("Permission Denied", "You do not have permission to add products.")
            return
    ...
    def update_product(self):
        if not self.check_permission('update'):
            messagebox.showerror("Permission Denied", "You do not have permission to update products.")
            return
        selected = self.view.products_tree.selection()
        if selected:
            item = self.view.products_tree.item(selected[0])
    ...
    def delete_product(self):
        if not self.check_permission('delete'):
            messagebox.showerror("Permission Denied", "You do not have permission to delete products.")
            return
        selected = self.view.products_tree.selection()
        if selected:
            item = self.view.products_tree.item(selected[0])
    ...
    def add_supplier(self):
        if not self.check_permission('add'):
            messagebox.showerror("Permission Denied", "You do not have permission to add suppliers.")
            return
    ...
    def update_supplier(self):
        if not self.check_permission('update'):
            messagebox.showerror("Permission Denied", "You do not have permission to update suppliers.")
            return
        selected = self.view.suppliers_tree.selection()
        if selected:
            item = self.view.suppliers_tree.item(selected[0])
    ...
    def delete_supplier(self):
        if not self.check_permission('delete'):
            messagebox.showerror("Permission Denied", "You do not have permission to delete suppliers.")
            return
        selected = self.view.suppliers_tree.selection()
        if selected:
            item = self.view.suppliers_tree.item(selected[0])
    ...
    def add_warehouse(self):
        if not self.check_permission('add'):
            messagebox.showerror("Permission Denied", "You do not have permission to add warehouses.")
            return
    ...
    def update_warehouse(self):
        if not self.check_permission('update'):
            messagebox.showerror("Permission Denied", "You do not have permission to update warehouses.")
            return
        selected = self.view.warehouses_tree.selection()
        if selected:
            item = self.view.warehouses_tree.item(selected[0])
    ...
    def delete_warehouse(self):
        if not self.check_permission('delete'):
            messagebox.showerror("Permission Denied", "You do not have permission to delete warehouses.")
            return
        selected = self.view.warehouses_tree.selection()
        if selected:
            item = self.view.warehouses_tree.item(selected[0])
    ...
    def search_inventory(self):
        if not self.check_permission('view'):
            messagebox.showerror("Permission Denied", "You do not have permission to view inventory.")
            return
    ...
    def show_low_stock(self):
        if not self.check_permission('view'):
            messagebox.showerror("Permission Denied", "You do not have permission to view low stock.")
            return
        for item in self.view.inventory_tree.get_children():
            self.view.inventory_tree.delete(item)
    ...
    def add_stock_entry(self):
        if not self.check_permission('add'):
            messagebox.showerror("Permission Denied", "You do not have permission to add stock entries.")
            return
        ...
        if not all([product, warehouse, quantity, date, trans_type]):
            messagebox.showerror("Error", "All fields are required.")
            return

        try:
            # Trích xuất ID từ combobox
            product_id = int(product.split(":")[0])
            warehouse_id = int(warehouse.split(":")[0])
            quantity = int(quantity)

            # Xử lý số lượng âm cho Sale/Adjustment
            ...
            try:
                ...
                messagebox.showerror("Error SovietError", "Invalid date format. Use YYYY-MM-DD.")
                return

            # Thêm bản ghi stock entry
            if self.stock_entry_model.add_stock_entry(product_id, warehouse_id, abs(quantity), date):
                # Thêm bản ghi lịch sử giao dịch
                if self.history_model.add_history_entry(product_id, warehouse_id, quantity, trans_type, date):
                    # Cập nhật giao diện
                    self.load_stock_entries()
                    self.load_inventory()
                    self.view.clear_stock_entry_fields()
                    messagebox.showinfo("Success", f"Stock entry ({trans_type}) added successfully.")
                else:
                    messagebox.showerror("Error", "Failed to record transaction history.")
            else:
                messagebox.showerror("Error", "Failed to add stock entry.")

        except ValueError as e:
            messagebox.showerror("Error", "Invalid input: Please check quantity or selection.")
        except Exception as e:
            messagebox.showerror("Error", f"An error occurred: {str(e)}")

    def show_low_stock_report(self):
        if not self.check_permission('view'):
            messagebox.showerror("Permission Denied", "You do not have permission to view reports.")
            return
